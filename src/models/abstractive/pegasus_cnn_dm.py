...
def pegasus_summarize(texts: List[str], max_length: int = 128, device: str | None = None) -> List[str]:
    try:
        from transformers import PegasusForConditionalGeneration, PegasusTokenizer
        import torch

        model_name = "google/pegasus-cnn_dailymail"
        tok = PegasusTokenizer.from_pretrained(model_name)
        ...
        raise RuntimeError("需要安裝 transformers/torch 才能使用 Pegasus") from e
