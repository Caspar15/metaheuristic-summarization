...
from src.models.extractive.roberta_rank import RobertaRanker
...
class ThreeStageRobertaSelector:
    """
    三階段選句器：NSGA-II 選 20 句 → RoBERTa 選 20 句 → MMR 選 3 句
    """
    def __init__(self, config):
        self.config = config
        ...
        # 第二階段：RoBERTa
        try:
            roberta_config = self.stage_config.get("roberta", {})
            model_name = roberta_config.get("model_name", "roberta-base")
            self.roberta_ranker = RobertaRanker(model_name)
            self.use_roberta = True
            print("RoBERTa ranker initialized successfully")
        except Exception as e:
            print(f"Warning: RoBERTa not available: {e}")
    ...
    def select(self, sentences, scores, sim_matrix, length_controller):
        """執行三階段選句"""
        print(f"\n=== Three-Stage RoBERTa Selection Process ===")
        ...
        # 階段 2：RoBERTa 獨立選出 20 句候選
        stage2_candidates = self._stage2_roberta_selection(sentences, scores)
        
        # 階段 3：MMR 從兩個階段的結果中選出最終句子
    ...
    def _stage2_roberta_selection(self, sentences, scores):
        """階段 2：使用 RoBERTa 獨立選出 20 句候選"""
        target_candidates = self.stage_config.get('stage2_candidates', 20)
        target_candidates = min(target_candidates, len(sentences))
        
        if not self.use_roberta:
            print("Stage 2: RoBERTa not available, using score-based selection")
            top_indices = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)
            return top_indices[:target_candidates]
        ...
        try:
            ...
            # RoBERTa 對所有句子進行評分和排序
            all_indices = list(range(len(sentences)))
            roberta_candidates = self.roberta_ranker.rank_sentences(
                sentences, all_indices, context, target_candidates
            )
            
            print(f"Stage 2 (RoBERTa): Selected {len(roberta_candidates)}/{target_candidates} candidates")
            return roberta_candidates
    ...
    def _stage3_mmr_fusion(self, sentences, stage1_candidates, stage2_candidates, sim_matrix, length_controller):
        ...
        print(f"Stage 3: MMR fusion from {len(stage1_candidates)} NSGA-II + {len(stage2_candidates)} RoBERTa = {len(combined_candidates)} unique candidates")
        ...
        for idx in combined_candidates:
            ...
            if idx in stage2_candidates:
                roberta_rank = stage2_candidates.index(idx)
                roberta_score = 1.0 - (roberta_rank / len(stage2_candidates))
                base_score += 0.5 * roberta_score
...
if __name__ == "__main__":
    print("--- Testing ThreeStageRobertaSelector ---")
    ...
    try:
        selector = ThreeStageRobertaSelector(config)
