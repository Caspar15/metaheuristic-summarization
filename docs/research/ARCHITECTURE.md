...
> 程式已進 master；兩 primary frozen-dev non-PLM 各 23/23 已完成。Multi-News P08
> macro 高 proposed S02b `0.003663`；GovReport LexRank 高 S02b `0.033758`。PacSum
> 上游 repo／checkpoint 因缺授權與 digest 不直接 vendoring（F-45），改以論文公式實作並強制
> 標示 protocol adaptation。PLM family、greedy reference、paired
