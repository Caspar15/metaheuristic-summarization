...
| Phase 3（方法實驗） | lexical/objective、graph 與兩-primary semantic 原 screens 已完成（保留 structural failures/follow-ups）。Multi-News S02b macro `0.328077` 首次高 Lead 點估計，但非 semantic 純 ablation；GovReport S00 `0.407203` 高 graph、仍低 Random／全文 lexical且成本 15.15×。semantic-direct selector 跨資料集刪除。仍缺 GovReport S02b、paired inference 及 **候選池對 greedy reference 的 recall@K** |
