...
| Phase 3（方法實驗） | D1 兩 primary lexical/objective 各 12/12 已完成。全文候選對 base：Multi-News `+0.010559` 但仍輸 Lead；GovReport `+0.045200` 且 dev point estimate 高於 Lead／Random。兩者共同顯示 candidate recall 是優先病因、全文搜尋不是最終答案。其餘四個 dataset×family cells、paired inference 與 **候選池對 greedy reference 的 recall@K** 仍待完成 |
