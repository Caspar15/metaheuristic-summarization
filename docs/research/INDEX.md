...
hash 全部相同；加入兩 primary partition／greedy-reference／GovReport data-layer／A1/D1 runners／D1 inventory／document-aware position與 F-30 regression 後 357 tests passed。SBERT centroid-only 與 full-source SBERT-MMR
...
尚未完成 full-source MMR、PacSum 或兩 primary Gate 2，
...
| D1 敏感度 | 🟡 19 組／90 個 effective config paths 已盤點；27 個 dev-only Greedy configs 已於分數前預註冊。本階段不看 dev-test。兩 primary lexical/objective 與 Multi-News cheap-multiroute 共三格各 12/12 已完成；GovReport cheap-multiroute 為 11 success + 1 uncapped section-guard structural failure，獨立預註冊的 cap-aware G11b 已完成且 681/681 feasible。兩資料集 graph hard pool 都明顯高於純 lexical base，但尚未一致勝過便宜 baseline；仍缺兩個 semantic cells、強 baseline 與 paired significance |
