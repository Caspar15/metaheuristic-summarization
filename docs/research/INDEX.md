...
| D1 敏感度 | 🟡 19 組／90 個 effective config paths 已盤點；27 個 dev-only Greedy configs 已於分數前預註冊。本階段不看 dev-test。兩 primary lexical/objective 與 Multi-News cheap-multiroute 共三格各 12/12 已完成；GovReport cheap-multiroute 為 11 success + 1 uncapped section-guard structural failure，cap-aware G11b 已獨立預註冊。兩資料集 graph hard pool 都明顯高於純 lexical base，但尚未一致勝過便宜 baseline；仍缺兩個 semantic cells、G11b、強 baseline 與 paired significance |
