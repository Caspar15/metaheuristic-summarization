...
hash 全部相同；加入兩 primary partition／greedy-reference／GovReport data-layer／A1 runner regression 後 332 tests passed。SBERT centroid-only 與 full-source SBERT-MMR
...
| 測試 | ✅ **332 local tests 全過**（2026-08-08）；PR #15 Linux CI 綠燈（2026-08-05），CI 已接 GitHub Actions |
...
| A1 長度協定 | 🟡 兩 primary dev 四候選 × 三方法皆完成；floor-bearing protocol 均暫列第一，但差距來自 Greedy stopping。Multi-News 有 4/3,935 可行 floor shortfalls（F-23）；GovReport floor-bound lexical Greedy 仍輸 Lead/Random（F-24）。兩資料集各一次 dev-test 尚未完成 |
