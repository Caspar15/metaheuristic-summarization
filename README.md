...
- `d3a-router-fusion-screen-v1` 已在任何新分數前凍結。兩個 task profile 各 14 案，
  先跑 reference-blind 200-row dev pilot，再依固定規則最多送四個非 anchor 到 full dev。
- `candidates.route_weights` 未設定時三路皆為 `1.0`，不改舊配置；非法／disabled route
  ...
  worker 限一個 BLAS thread。dev-test/test 沒有入口；目前尚無 D3a 分數。
...
| 測試 | ✅ **419 local tests passed**（2026-08-09）；PR #15 Linux CI 綠燈（2026-08-05），CI 維持 push／PR 自動執行 |
