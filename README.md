...
- 真實 canonical 3-row correctness/cost smoke 已通過 matched hashes；不含 ROUGE、
  ...
  D1 inventory 與 document-aware position regression 後，現行測試為 **338 passed**。
...
- A1 兩資料集的 reference-only 統計與候選協定已在分數前預註冊；study runner 已
  ...
  唯一一次 A1 dev-test 都已完成並凍結；partitioned baseline matrix、PacSum 與正式方法
  runs 仍缺；test split 仍鎖定。
...
- D1 dev-only Greedy 敏感度研究已在任何新分數前預註冊：完整盤點 19 組／90 個
  ...
  document-aware position correctness 已修復（F-25），下一步跑 27 configs × 兩 primary；本 screen 不看
...
| 測試 | ✅ **338 local tests passed**（2026-08-08）；PR #15 Linux CI 綠燈（2026-08-05），CI 維持 push／PR 自動執行 |
