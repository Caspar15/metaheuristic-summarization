...
本專案是一個模組化抽取式摘要系統，預設以 CNN/DailyMail 類型資料為測試場景。提供基線規則特徵（TF‑ISF、長度、句位）與多種選句優化（Greedy/GRASP/NSGA‑II），並可選擇使用生成式模型（BART/Pegasus）做對照實驗。

目前統一以「前處理 → 選句 → 評估」三段式命令使用。監督式（supervised）相關模組已移除；候選池 top‑k 與 `representations.use` 的行為已落地。
...
- 中文長度控制失真：
  ...
  - 臨時解法：降低 `max_tokens`；後續將提供 `unit: chars` 與 `tokenizer` 選項。
