...
- 新增二階段 rerank 的檔案預留（現移至 experimental/；見 docs）。
- 新增不訓練 BERT 排序器（`optimizer: bert`）：以 `bert-base-uncased` 句向量對文件重心 cosine 排序；支援三句長度約束。
- 新增分數融合 + MMR 選句（`optimizer: fused`）：`S = w_base*base + w_bert*bert`，以 BERT 相似度做冗餘抑制。
- 新增二階段做法 B：兩路各取 Top‑K（K1：基線；K2：BERT）→ 聯集 U → 最終決策（BERT 或 fused）。
- 新增腳本：`scripts/build_union_stage2.py`（建 U）、`scripts/tune_union_fusion.py`（小網格調參）；調參 YAML 產出改至 `configs/_generated/tune-<stamp>/`。
- 修正：聯集 cap 取前 N 時，先以基線分數排序再截斷，避免索引偏差。
- 補充：若遇 Hugging Face 429 限流，建議 `huggingface-cli login` 或設 `HUGGINGFACE_TOKEN`/`HF_TOKEN`。
...
- 腳本與工具（Scripts）
  - 檔案：`scripts/{benchmark_small.py,split_dataset.py,jsonl_to_csv.py,run_7_2_1.py,build_union_stage2.py,tune_union_fusion.py}`
  - 功能：資料切分/轉換，整合跑流程，小樣本基準；二階段聯集與小網格調參。

- 新增最佳化器（Optimizers）
  - 檔案：`src/models/extractive/{bert_rank.py,fused.py}`
  - 功能：BERT 排序器（不訓練）；分數融合 + MMR。
...
- D Score & Output
- D1 2nd‑stage scoring：提供 BERT 排序器與（可選）fused 最終決策；cross‑encoder 仍為預留（見下 Gaps 3）
...
- 檔案預留：`experimental/rerank/cross_encoder.py`、`experimental/pipeline/rerank.py`、`configs/features_basic.yaml` 的 `rerank` 區塊。
 - 已提供：以 BERT 排序器與 fused 在聯集 U 上做最終決策的二階段實作（非 cross‑encoder）。
...
（詳細使用與輸出請見：`docs/PIPELINE.md`, `docs/RUNS.md`, `docs/CONFIGS.md`）
...
- 選句與產生摘要
  ...
  - BERT 排序（三句）：`python -m src.pipeline.select_sentences --config configs/features_bert_3sent.yaml --split validation --input data/processed/validation.jsonl --run_dir runs --optimizer bert`

- 二階段（做法 B）精簡：
  - K1（基線，20 句）：`python -m src.pipeline.select_sentences --config configs/features_20sent.yaml --split validation --input data/processed/validation.jsonl --run_dir runs --optimizer greedy --stamp stage1-greedy-top20`
  ...
  - 最終 3 句（BERT on U）：`python -m src.pipeline.select_sentences --config configs/features_bert_3sent.yaml --split validation --input data/processed/validation.stage2.union.jsonl --run_dir runs --optimizer bert --stamp stage2-bert-top3`
...
- CNN/DailyMail validation（sample=100；二階段做法 B）：
  - BERT 在聯集 U 上直選（K1=20, K2=20, cap=20）：約 RL≈0.2096（本機實驗；僅供相對比較）
  - fused（w_bert=0.5, α=0.7，cap=25）：約 RL≈0.2049（仍需調參）
